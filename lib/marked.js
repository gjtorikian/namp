...
var phpMarkdown = require('./php_markdown');
...
block.token = function(src, tokens, top) {
  ...
  while (src) {
    ...
    c = phpMarkdown.php_extras_token(cap, block, src, tokens);

    if (c) {
      c = src.substring(cap[0].length);
      continue;
    }
...
var marked = function(src) {
  var php_extras_block = phpMarkdown.php_extras_obj;

  // is this the best place to do this?
  for (var attrname in php_extras_block) 
  { 
      block[attrname] = php_extras_block[attrname]; 
  }
